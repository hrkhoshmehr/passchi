...
const Schema = z.object({
   ...
   */
  /**
   * سقفِ هفتگیِ سهم‌هایی که هم‌کلاسی‌ها با **اعتبارِ هدیه** می‌دهند، به تومان
   * (`billing/sharing.ts`). بی آن، حساب‌های ساختگی هدیه‌شان را از راهِ سهم به
   * اعتبارِ واقعیِ یک نفر تبدیل می‌کنند. پر که شد، فقط با اعتبارِ خریداری‌شده.
   */
  SHARE_GIFT_TOMAN_PER_WEEK: num(1_000_000),
  ...
  /**
   * هدیهٔ شروع برای هر حسابِ تازه، به تومان.
   *
   * عمداً کم: کارش این است که تازه‌وارد بتواند **سهمِ جلسهٔ یک هم‌کلاسی** را
   * بردارد، نه یک کلاس کامل — فایلِ اولِ خودش جداگانه رایگان است (`free-file.ts`).
   */
  FREE_TRIAL_TOMAN: num(20_000),
