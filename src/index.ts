 ...
 *
 * **پشتیبانِ دوباره‌تلاش.** خطای شبکه را خودِ grammY در `start()` با `withRetries`
 * دوباره تلاش می‌کند (۲۰۲۶-۰۹-۱۴ که بله از سرور در دسترس نبود همین را دیدیم؛
 * «Aborted delay» فقط `stop()` وسطِ همان انتظار بود). ولی خطاهایی که grammY
 * پرتاب می‌کند — مثل ۴۰۱ یا ۴۰۹ — polling را برای همیشه می‌خواباندند؛ این حلقه
 * آن‌ها را هم با فاصلهٔ تا پنج دقیقه دوباره امتحان می‌کند.
 */
