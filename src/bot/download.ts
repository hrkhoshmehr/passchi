...
/**
 * grammY نوعِ `AbortSignal` را از بستهٔ پالیفیلِ `abort-controller` می‌گیرد، نه
 * از خودِ Node — دو تعریفِ ساختاریِ ناسازگار برای یک چیزِ یکسان. تبدیل فقط
 * همین‌جا یک بار انجام می‌شود تا صدازننده‌ها تمیز بمانند.
 */
type ApiSignal = NonNullable<Parameters<Api["getFile"]>[1]>;

/**
 * یک فراخوانیِ Bot API با مهلتِ **خودمان**.
 *
 * پیش‌فرضِ grammY پانصد ثانیه است. برای `sendMessage` عدد بی‌ضرری است، ولی
 * `getFile` اولین گامِ هر تلاشِ دانلود است و روی همان سروری می‌نشیند که
 * می‌دانیم یک‌سوم مواقع جواب نمی‌دهد. اگر آن حالت به‌جای «رد کردن» به شکل
 * «هرگز جواب ندادن» ظاهر شود، یک تلاش هشت دقیقه طول می‌کشد — و چون
 * `bot.start()` آپدیت‌ها را پشت سر هم پردازش می‌کند، در آن هشت دقیقه کلِ ربات
 * می‌خوابد، نه فقط کارِ همین کاربر.
 */
export async function callWithDeadline<T>(
  ms: number,
  why: string,
  fn: (signal: ApiSignal) => Promise<T>,
): Promise<T> {
  const ac = new AbortController();
  const t = setTimeout(() => ac.abort(new Error(why)), ms);
  t.unref?.();
  try {
    return await fn(ac.signal as unknown as ApiSignal);
  } finally {
    clearTimeout(t);
  }
}
...
async function viaBotApi(
  ...
  /**
   * سه مهلتِ جدا، چون سه شکستِ جدا وجود دارد.
   *
   ...
   * شروع می‌شود.
   *
   * همان عدد روی `getFile` هم می‌نشیند — که پیش از این هیچ مهلتی از سمت ما
   * نداشت و به پیش‌فرضِ پانصدثانیه‌ای grammY تکیه می‌کرد.
   *
  ...
  const file = await callWithDeadline(HEADER_MS, "سرور به درخواست فایل جواب نداد.", (signal) =>
    api.getFile(req.fileId, signal),
  );
