...
export const WELCOME = `به پاسچی خوش اومدی 👋

دیگه لازم نیست وقتتو تو کلاس تلف کنی. فقط صوت کلاسو برام بفرست، بهت می‌گم استاد چیکارا کرده تو کلاس — و اگه یه‌جایی گفته «امتحان می‌آد» یا تکلیف داد، اونم با عین جمله‌ش بهت می‌گم.

که خیالت راحت باشه: صوت کلاس با دقت، کلمه‌به‌کلمه پیاده می‌شه.

🎁 اولش رایگانه — تا ${toFaDigits(config.FREE_TRANSCRIPT_MINUTES)} دقیقه صوت یا ویس بفرست، مهمون من.

<i>سوالی هم داشتی${config.SUPPORT_USERNAME ? `، به @${config.SUPPORT_USERNAME} پیام بده` : ""}.</i>`;
