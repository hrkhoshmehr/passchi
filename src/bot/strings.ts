 ...
 *
 * **زیرنویس فقط برای ویدیو معنا دارد**، و برای کلاسِ صوتی فایلی است که گوشی
 * بازش نمی‌کند. بهتر بود فقط برای جلسهٔ ویدیویی ساخته شود، ولی جلسه هیچ
 * نشانِ قابل‌اتکایی از ویدیو بودنِ منبع ندارد: نوعِ پیام جایی ذخیره نمی‌شود،
 * پسوندِ فایل روی دیسک ممکن است عوض شده باشد، و خودِ فایل پس از
 * `KEEP_AUDIO_DAYS` پاک می‌شود. پس برچسب خودش می‌گوید به چه کاری می‌آید تا
 * کسی که ویدیو ندارد نزندش.
 */
export const MORE_BTN = {
  timeline: "🕘 کلاس دقیقه‌به‌دقیقه",
  transcript: "📄 متن کامل کلاس",
...
export const MORE_PROMPT =
  "📎 <b>متن کامل کلاس و کلاس دقیقه‌به‌دقیقه هم آماده‌ست</b>\n" +
