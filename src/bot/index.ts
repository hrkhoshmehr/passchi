...
handlers.command("start", async (ctx) => {
  ...
  /**
   * لینکِ قدیمیِ خرید گروهی: /start p_<sessionId>
   *
   * خرید گروهیِ پیش از پرداخت برداشته شد (فایلِ اول رایگان است و شریک‌شدن پس
   * از تحویل همان کار را ساده‌تر می‌کند). لینک‌هایی که در گروه‌ها مانده‌اند
   * جوابِ روشن می‌گیرند، نه منوی بی‌ربط.
   */
  if (payload.startsWith("p_")) {
    await reply(ctx, S.GROUP_LINK_GONE, { reply_markup: mainKeyboard });
    return;
  }
