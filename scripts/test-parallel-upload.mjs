...
// ─── ادعاها روی کد واقعی ────────────────────────────────────────────────────
{
  ...
  /**
   * **`uploads.set` باید پیش از هر `await` باشد.**
   *
   * وگرنه تکه‌های موازی که هم‌زمان `get` را خالی می‌بینند، هر کدام یک
   * `prog`ِ جدا می‌سازند و بازه‌ها روی هم جمع نمی‌شوند — کار هرگز بسته
   * نمی‌شود. این باگ همهٔ آپلودهای موازی را زمین می‌زد و فقط لاگِ CDN
   * (همه ۲۰۰، ولی هیچ `sessionId`) لو دادش. اینجا ساختار را قفل می‌کنیم:
   * بینِ ساختِ `prog` و `set`اش نباید `await` باشد.
   */
  const setIdx = srvTs.indexOf("uploads.set(key, prog)");
  const statIdx = srvTs.indexOf("await fsp.stat(part)");
  ok("set پیش از await fsp.stat است", setIdx >= 0 && statIdx > setIdx);
