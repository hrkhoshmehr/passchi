...
// اگر کسی این‌ها را عوض کرد، باید بداند چه چیزی را می‌شکند.
{
  ...
  /**
   * سقفِ زمان دیگر از **شمارشِ** تلاش‌ها درنمی‌آید.
   *
   * فرمول قبلی «هر تلاش حدود مهلتِ سرآیند» را ضربدر تعداد می‌کرد. از وقتی
   * تلاش دوباره بودجهٔ زمانی گرفت و سقفِ شمارش فقط بندِ ایمنی شد، همان ضرب
   * ۱۴۱ ثانیه می‌داد در حالی که کد هرگز بیش از بودجه تلاش نمی‌کند — قرمزیِ
   * کاذبی که ربطی به رفتار واقعی نداشت.
   *
   * بدترین حالتِ واقعی: بودجه تمام می‌شود و آخرین تلاشِ شروع‌شده هم تا
   * مهلتِ سرآیندش ادامه می‌یابد.
   */
  const budget = Number(idx.match(/const TRY_BUDGET_MS = ([\d_]+)/)?.[1]?.replace(/_/g, ""));
  ok("بودجهٔ زمانی تلاش دوباره تعریف شده", Number.isFinite(budget), String(budget));
  ok(
    "کل زمان بدترین حالت زیر دو دقیقه می‌ماند",
    budget + header <= 120000,
    `${((budget + header) / 1000).toFixed(0)} ثانیه — بودجه به‌علاوهٔ آخرین تلاش`,
  );
